...
void setup() {
  ...
  if (digitalRead(wipeB) == LOW) {  // when button pressed pin should get low, button connected to ground
    ...
    if (buttonState == true && digitalRead(wipeB) == LOW) {    // If button still be pressed, wipe EEPROM
      ...
      Serial.println(F("formatada com sucesso"));
  ...
  if (programMode) {
    if ( isMaster(readCard) ) { //When in program mode check First If master card scanned again to exit program mode
      ...
      if ( findID(readCard) ) { // If scanned card is known delete it
        ...
        deleteID(readCard);
        Serial.println("-----------------------------");
        Serial.println(F("Leia um chip para adicionar ou remover "));
      }
...
void ShowReaderDetails() {
  ...
  if (v == 0x91)
    ...
    Serial.print(F(" (desconhecido),provavelmente um clone?"));
  Serial.println("");
  // When 0x00 or 0xFF is returned, communication probably failed
  if ((v == 0x00) || (v == 0xFF)) {
    Serial.println(F("ALERTA: Falha na comunicacao, o modulo  esta conectado corretamente?"));
...
void writeID( byte a[] ) {
  if ( !findID( a ) ) {     // Before we write to the EEPROM, check to see if we have seen this card before!
    ...
    EEPROM.commit();
    successWrite();
    Serial.println(F("ID adicionado com sucesso"));
  }
  else {
    failedWrite();
    Serial.println(F("Erro! Tem alguma coisa errada com o ID do chip ou problema "));
  }
}
